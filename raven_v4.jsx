...
export default function RingFinder() {
  ...
  const [collectorName, setCollectorName] = useState("Your Name");
  const [collectorSize, setCollectorSize] = useState("Your Size");
  ...
  return (
    <div style={{ minHeight: "100vh", background: "#0D0C09", color: "#E8DFC8", fontFamily: "Georgia,'Times New Roman',serif" }}>
          ...
          <div>
            ...
            {editingCollector ? (
              <div style={{ display: "flex", gap: 6, alignItems: "center", flexWrap: "wrap" }}>
                <input className="input" value={collectorName} onChange={e => setCollectorName(e.target.value)}
                  placeholder="Your name" style={{ width: 120, padding: "3px 7px", fontSize: 11 }} />
                <input className="input" value={collectorSize} onChange={e => setCollectorSize(e.target.value)}
                  placeholder="Ring size" style={{ width: 80, padding: "3px 7px", fontSize: 11 }} />
                <button onClick={() => setEditingCollector(false)}
                  style={{ background: "#C8A96E", border: "none", color: "#0D0C09", padding: "3px 10px", fontSize: 10, cursor: "pointer", letterSpacing: ".08em" }}>
                  DONE
                </button>
              </div>
            ) : (
              <p onClick={() => setEditingCollector(true)}
                style={{ fontSize: 10, color: "#4A4030", letterSpacing: ".1em", textTransform: "uppercase", cursor: "pointer" }}
                title="Click to edit">
                {collectorName} · {collectorSize} · Snake · Signet · Intaglio · Mourning · Matte Gold ✎
              </p>
            )}
          </div>
          ...
          <div>
            ...
            {queued.length === 0 ? (
              ...
              <div style={{ marginBottom: 16 }}>
                <div className="lbl" style={{ marginBottom: 5 }}>CSV — select all and copy, then paste into Numbers or Excel</div>
                <textarea readOnly value={exportText}
                  onClick={e => e.target.select()}
                  style={{ width: "100%", height: 120, background: "#0A0908", border: "1px solid #C8A96E0C", color: "#6A5A40", fontFamily: "'Courier New',monospace", fontSize: 10, padding: "10px 12px", resize: "vertical", lineHeight: 1.7 }} />
              </div>
              {queued.map((ring, i) => (
      ...
      <div style={{ borderTop: "1px solid #C8A96E08", padding: "11px 34px", display: "flex", justifyContent: "space-between", fontSize: 10, color: "#222018", letterSpacing: ".05em" }}>
        <span>Ring Finder v3 · {collectorName} · {collectorSize} · 16k token search</span>
